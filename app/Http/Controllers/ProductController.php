...
class ProductController extends Controller
{
    ...
    public function addpettycash(Request $request)
    {
        ...
        return view('home', compact('products', 'cash', 'stockminims', 'label', 'value'));
    }
        ...
        $today = date("l");
            ...
            foreach ($period as $key => $val) {
                ...
                // $company = Lunas::whereDate('tanggal_pemesanan',$val->format('Y-m-d'));                    
                    ...
                    // ->groupBy(DB::raw('WEEK(created_at)'))
                    // ->get();
                    $company =  Lunas::select(DB::raw('DAY(tanggal_pemesanan) as week_number, COUNT(*) as total'))
                ->groupBy(DB::raw('WEEK(tanggal_pemesanan)'))
                ->get()->toArray();
